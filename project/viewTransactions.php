...
<?php
    if(isset($_POST["save"])){
        ...
        $stmt = $db->prepare("SELECT act_dest_id, Accounts.id, Accounts.account_number, amount, action_type, memo FROM Transactions JOIN Accounts on Accounts.id = Transactions.act_src_id WHERE act_src_id =:id AND action_type=:action_type AND created BETWEEN :startDate AND :endDate LIMIT 10");
