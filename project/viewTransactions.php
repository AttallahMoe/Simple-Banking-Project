...
if(isset($transId)){
    $db = getDB();
    $stmt = $db->prepare("SELECT act_src_id, Accounts.id, Accounts.account_number, amount, action_type, memo FROM Transactions ORDER BY created JOIN Accounts on Accounts.id = Transactions.act_dest_id WHERE act_src_id =:id ORDER BY Transactions.created LIMIT 10");
